...
for i in range(bushes):
    berry.append(random.randint(1,5))
print(berry)
max_berry = 0
for i in range(bushes):
    berry_current = berry[i]
    
    left_bush = i-1
    if i-1 < 0: 
        left_bush = bushes - 1
    right_bush = i+1
    if right_bush > bushes-1:
        right_bush = 0

    berry_sum = berry_current + berry[left_bush] + berry[right_bush]

    if berry_sum > max_berry:
        max_berry = berry_sum
print (max_berry)
